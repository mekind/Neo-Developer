...
├── frame-map.json      ← frontend용 애니메이션 좌표 메타데이터
...
- **character.png**: 프론트엔드가 화면에 그리는 자산. LPC 표준 layout(13행×N열, 64×64 frame)이지만, 현재 frontend 정책은 **walk-only**다. `walk_n/w/s/e` 행만 authoritative 하며, idle 행은 layered clothing 누락 위험 때문에 사용하지 않는다.
- **frame-map.json**: frontend용 애니메이션 좌표 메타데이터. 파일에는 `walk_*`와 `idle_*`가 모두 들어가지만, frontend는 `walk_*`만 사용한다.
- **lpc-state.json**: 같은 캐릭터를 다시 만들거나 수정할 때 입력. LPC UI에 paste하면 동일 결과
- **CREDITS.txt**: 화면 어딘가에 표기 또는 별도 페이지에 포함. 빠지면 라이선스 위반

### frame layout

LPC native라 모든 캐릭터가 같은 layout이지만, 현재 운영 정책은 다음과 같다.

- frontend는 `walk_n/w/s/e`만 재생한다.
- idle 행(`idle_n/w/s/e`)은 문서상 좌표가 남아 있어도 **non-authoritative** 로 취급한다.
- 정지 자세가 필요하면 idle 행 대신 **첫 walk frame 고정** 또는 **walk-derived pseudo-idle** 을 사용한다.
- `frame-map.json`은 생성물에 포함되며, frontend는 그중 walk subset만 소비하면 된다.
...
| Composer (Phase 1) | lpc-state.json (사용자가 paste) | character.png, frame-map.json, lpc-state.json (round-trip), CREDITS.txt |
| Composer (Phase 2) | lpc-state.json | 위와 동일 (자동) |
| Frontend 인계 | — | character.png + frame-map.json + lpc-state.json + CREDITS.txt |
...
- per-character custom frame-map schema 확장
...
- **v2 (2026-04-26)** — LPC native enum 채택 / Import-from-Clipboard / frontend 인계 4파일(character.png + frame-map.json + lpc-state.json + CREDITS.txt). 현재 frontend animation policy는 walk-only이며, idle rows는 non-authoritative 로 취급한다.
