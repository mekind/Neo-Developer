...
python backend/lpc-character-pipeline/scripts/generate_character.py \
    ...
    backend/lpc-character-pipeline/output/news-bot
```
...
Flags:
- `--catalog PATH` — override curated catalog (default `backend/lpc-character-pipeline/poc/lpc-catalog-curated.json`)
...
- Catalog (`backend/lpc-character-pipeline/poc/lpc-catalog-curated.json`, 89 KB / ~22 K tokens) is sent to Gemini in full.
...
- See `backend/lpc-character-pipeline/spec.md` for full module spec; `backend/lpc-character-pipeline/poc/` for manual dry-run procedure.
